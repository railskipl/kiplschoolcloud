...
#    render :partial => "new_entry"
  ...
  def tte_from_batch_and_tt(tt)
    ...
    @ele_subjects = Subject.find_all_by_batch_id(@batch.id, :conditions=>["elective_group_id IS NOT NULL AND is_deleted = false"])
    @ele_group = @ele_subjects.group_by {|types| types.elective_group_id}
  end
