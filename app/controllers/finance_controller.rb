...
class FinanceController < ApplicationController
    ...
    if params[:query].length>= 3
      @employee = Employee.find(:all,
        :conditions => ["(first_name LIKE ? OR middle_name LIKE ? OR last_name LIKE ?
                       OR employee_number LIKE ? OR (first_name || ' ' || last_name) LIKE ?)" + other_conditions,
  ...
  def load_fees_submission_batch
    ...
    @student ||= @fee.student
     ...
     if @student.present?
      @bus_fee_not_paid =  (@student.bus_service.present? and not(@student.bus_fee_paid))  
      @hostel_fee_not_paid = (@student.room.present?  and not(@student.hostel_fee_paid))
      @iterator = []

      if @bus_fee_not_paid
        @iterator << {:text => "Bus Fee",:class => "bus_fee_paid",:amount => BusSetting.bus_charge(@start_date,@end_date) ,:paid => false}
      elsif @student.bus_fee_paid
        @iterator << {:text => "Bus Fee",:class => "bus_fee_paid",:amount => BusSetting.bus_charge(@start_date,@end_date),:paid => true}
      else
        @iterator << {:text => "Bus Fee",:class => "not",:amount => BusSetting.bus_charge}
      end  

      if @hostel_fee_not_paid
        @iterator << {:text => "Hostel Fee",:class => "hostel_fee_paid",:amount => @student.room.rent || 0,:paid => false} 
      elsif @student.hostel_fee_paid
        @iterator << {:text => "Hostel Fee",:class => "hostel_fee_paid",:amount => @student.room.rent || 0,:paid => true} 
      else
         @iterator << {:text => "Hostel Fee",:class => "not",:amount => 0} 
      end  

    end  

    Rails.logger.info "======================"
    Rails.logger.info @iterator.inspect
    Rails.logger.info "======================"
  ...
  def update_ajax
    ...
    if params[:fee].present?
      @student.update_attributes(params[:fee])
    end
      
    total_fees = extra_charges(total_fees)
    ...
    unless params[:fees][:fees_paid].to_f < 0
      ...
      unless params[:fees][:fees_paid].to_f > params[:total_fees].to_f
        ...
        transaction.title =  if (total_fees > params[:fees][:fees_paid].to_f ) 

            "#{t('receipt_no')}. (#{t('partial')}) F#{@financefee.id}" 

          else  
     
            "#{t('receipt_no')}. F#{@financefee.id}"
          end
  ...
  def student_fee_receipt_pdf
    ...
    if @student.present?
      ...
      if @bus_fee_not_paid
        @iterator << {:text => "Bus Fee",:class => "bus_fee_paid",:amount => BusSetting.bus_charge ,:paid => false}
      elsif @student.bus_fee_paid
        @iterator << {:text => "Bus Fee",:class => "bus_fee_paid",:amount => BusSetting.bus_charge,:paid => true}
      else
        @iterator << {:text => "Bus Fee",:class => "not",:amount => BusSetting.bus_charge}
      end  
    ...
    render :pdf => 'fee_receipt_pdf' ,:layout => "pdf"
  ...
  def pay_fees_defaulters
    ...
    if @student.present? and params[:fee].present?
      @student.update_attributes(params[:fee])
    end
  ...
  def extra_charges(total_fees)
    if @student.present?
      ...
      if @bus_fee_not_paid
        @iterator << {:text => "Bus Fee",:class => "bus_fee_paid",:amount => BusSetting.bus_charge ,:paid => false}
        total_fees += BusSetting.bus_charge
      elsif @student.bus_fee_paid
        @iterator << {:text => "Bus Fee",:class => "bus_fee_paid",:amount => BusSetting.bus_charge,:paid => true}
        total_fees += BusSetting.bus_charge
      else
