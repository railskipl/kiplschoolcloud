...
class Configuration < ActiveRecord::Base
  ...
  class << self
    ...
    def get_grading_types
      ...
      types= all(:conditions=>{:config_key=>grading_types.values, :config_value=>"true"})
      gtypes = types.group_by {|types| types.config_key}
      grading_types.keys.select{|k| gtypes.collect(&:config_key).include? grading_types[k]}      
    end
    ...
    def set_grading_types(updates)
      ...
      updates.each do |t|
        find_or_create_by_config_key(grading_types[t]).update_attribute(:config_value, true)
      end
      deletions.each do |t|
        find_or_create_by_config_key(grading_types[t]).update_attribute(:config_value, false)
      end
    end
    ...
    def cce_enabled?
      get_config_value("CCE") == "true"
    end

    def has_gpa?
      get_config_value("GPA") == "true"
    end

    def has_cwa?
      get_config_value("CWA") == "true"
    end
