...
class EmployeeController < ApplicationController
    ...
    if params[:query].length>= 3
      @employee = Employee.find(:all,
        :conditions => ["(first_name LIKE ? OR middle_name LIKE ? OR last_name LIKE ?
                       OR employee_number = ? OR (first_name || ' ' || last_name) LIKE ? )"+ other_conditions,
  ...
  def update_rejected_employee_list
    ...
    @employees = MonthlyPayslip.find(:all, :conditions =>"is_rejected is true", :joins=>"INNER JOIN employees on monthly_payslips.employee_id = employees.id")
    
    types = @employees.group_by {|ff| ff.employee_id}
