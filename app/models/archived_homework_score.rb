class ArchivedHomeworkScore < ActiveRecord::Base
  belongs_to :student
    belongs_to :homework
    ...
    def grouped_homework_subject_total(subject,student,type,batch = "")
     ...
     homework_groups.each do |homework_group|
       ...
       unless homework.nil?
       homework_score = ArchivedhomeworkScore.find_by_student_id(student.id, :conditions=>{:homework_id=>homework.id})
  ...
  check = HomeworkGroup.find_all_by_batch_id(batch.id)
  var = []
  check.each do |x|
  ...
  if var.empty?
      ...
      marks_total = 0
      homework_groups.each do |homework_group|
        max_total = max_total + homework_group.archived_total_marks(student)[1]
        marks_total = marks_total + homework_group.archived_total_marks(student)[0]
      end
