...
#
  ...
  def self.common_active
   self.find(:all , :conditions => ["finance_fee_categories.is_master = '#{true}' and finance_fee_categories.is_deleted = '#{false}'"], :joins=>"INNER JOIN batches on finance_fee_categories.batch_id = batches.id AND batches.is_active = '#{true}' AND batches.is_deleted = '#{false}' ")
  end
