...
class Student < ActiveRecord::Base
  ...
  named_scope :with_full_name_only, :select=>"id, (''||first_name||' '||last_name) AS name,first_name,last_name", :order=>:first_name
  named_scope :with_name_admission_no_only, :select=>"id, (''||first_name||' '||last_name||' - '||admission_no) AS name,first_name,last_name,admission_no", :order=>:first_name

  named_scope :by_first_name, :order=>'first_name',:conditions => { :is_active => true }

  validates_presence_of :admission_no, :admission_date, :first_name, :batch_id, :date_of_birth,:caste,:route_id
  ...
  validates_associated :user,:student_previous_data
