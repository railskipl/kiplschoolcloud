...
class FinanceTransaction < ActiveRecord::Base
  ...
  def self.total_transaction_amount(transaction_category,start_date,end_date)
    ...
    category_type = finance_transaction_category.is_income ? "income" : "expense" rescue "nil"
    transactions = FinanceTransaction.find(:all,
      :conditions => ["transaction_date >= '#{start_date}' and transaction_date <= '#{end_date}'and category_id ='#{finance_transaction_category.id}'"])rescue "nil"
    transactions.each {|transaction| amount += transaction.amount}rescue "nil"
