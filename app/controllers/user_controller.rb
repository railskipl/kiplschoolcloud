...
class UserController < ApplicationController
  ...
  def dashboard
     @schoolname = Configuration.find(1)
    ...
    if request.post? and params[:reset_password]
      if user = User.find_by_username(params[:reset_password][:username])
        unless user.email.blank?
          ...
          user.save(false)
          Rails.logger.info "The user is over here .........."
          Rails.logger.info "The user is over here .........."
          Rails.logger.info "The user is over here .........."
          Rails.logger.info "The user is over here .........."
          Rails.logger.info "The user is over here .........."
  ...
  if params[:device] == 'mobile' 
        ...
        if @user.parent == true
          ...
          authenticated_user = @user 
             ...
             render :status =>200,:json => { :error => "Invalid" } 
      ...
      @user = User.find(:all,
        ...
        # :order => "first_name asc") unless params[:query] == ''
           ...
           :conditions => ["first_name LIKE ? OR last_name LIKE ?
                            OR username = ? OR (first_name || ' ' || last_name) LIKE ?  ",
            "#{params[:query]}%","#{params[:query]}%",
            "#{params[:query]}", "#{params[:query]}" ], :order => "first_name asc") unless params[:query] == ''
      #      else
