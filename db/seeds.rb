...
if User.first( :conditions=>{:admin=>true}).blank?
  ...
  employee = Employee.find_or_create_by_employee_number(:employee_number => 'admin',:joining_date => Date.today,:first_name => 'Admin',:last_name => 'User',
    :employee_department_id => employee_department.id,:employee_grade_id => employee_grade.id,:employee_position_id => employee_position.id,:employee_category_id => employee_category.id,:status => true,:nationality_id =>'76', :date_of_birth => Date.today-365, :email => 'info@schoolcloud.in')
...
["General","OBC","ST","SC"].each do |category|
  StudentCategory.create(:name => category)
end

BusSetting.create(:name => "Default",:default_charge => true,:amount => 200)