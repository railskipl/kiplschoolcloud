...
class StudentController < ApplicationController
  ...
  before_filter :find_student, :only => [
    ...
    :guardians, :academic_pdf,:show_previous_details,:fees,:fee_details,:card_print,:print
  ]
  ...
  def search_ajax
    ...
    elsif params[:option] == "course"

    elsif params[:option] == "village"
      @students = []
      if params[:query].length>= 3
        route_ids = Route.find(:all,
          :conditions => ["LOWER(name) LIKE ?",
            "#{params[:query].downcase}%"]).collect(&:id)
        @students = Student.find(:all,:conditions => ["route_id IN (?)
          ",route_ids])
      end  
       render :layout => false
    end  
  ...
  def profile
    ...
    @bus_services = BusService.find(:all,:joins => :memberships,:conditions => ["memberships.route_id = ?",@student.route_id])
  ...
  def card_printing_ajax
    if params[:option] == "active"
      ...
      render :partial => "custom_printing_ajax"
    end
  end  
  ...
  def update_bus_service
    @student = Student.find(params[:id])
    respond_to do |format|
      if @student.update_attributes(params[:student])
        format.html { render :text => t('bus_service_text.update_successful') } 
      else
        format.html { render :text => "Error Updating BusService" }
      end  
    end
  end
  private
